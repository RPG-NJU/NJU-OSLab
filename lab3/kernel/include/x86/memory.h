...
struct ProcessTable {
	uint32_t stack[MAX_STACK_SIZE]; // 进程独立的内核栈区
	struct StackFrame regs; // 从用户态陷入内核态的时候，存储的所有寄存器（还有部分其他的信息）
	uint32_t stackTop; // 栈顶
	uint32_t prevStackTop; // XF老师说，这个变量在后续的迭代中已经被弃置了
	int state; // 进程的状态 RUNNING RUNNABLE BLOCKED DEAD
	int timeCount; // 记录每个进程的处理（RUNNING）时间片
	int sleepTime; // 记录每个进程阻塞（BLOCKED）的时间片
	uint32_t pid; // PID号
	char name[32]; // 进程名？
};
...
struct TSS {
	...
	uint32_t esp0;         // Ring 0 Stack pointer and segment selector Ring0堆栈指针和段选择器
	uint32_t ss0;          // after an increase in privilege level 在特权级别增加之后
	union{
		struct{
			char dontcare[88];
		};
