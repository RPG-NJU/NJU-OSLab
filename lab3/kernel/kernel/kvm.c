...
void initProc() 
{
	...
	// switch to kernel stack for kernel idle process
	
	enableInterrupt();
	
	asm volatile("int $0x20"); // trigger irqTimer
	// HELLORPG：这里会引发一个中断，转而执行irqHandle.c中的timerHandle函数，需要完成
	// 这里的本质其实是希望通过时间片用尽这个判定切换到用户进程
	// [GRP-01]
	// putChar('?');
	while(1)
		waitForInterrupt(); // hlt
}
